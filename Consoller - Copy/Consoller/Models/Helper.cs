...
namespace Consoller.Models
{
    public class Helper
    {
        ...
        {
            return HttpContext.Current.User.IsInRole("Receptionist") ? Receptionist() : HttpContext.Current.User.IsInRole("Franchisee") ? Franchisee() : "Admin";
        }
        ...
        // Recipt No For Applicant Fee
        public int recipt()
        {
            ApplicationRecipt rr = db.ApplicationRecipts.FirstOrDefault();
            if(rr==null)
            {
            ...
            {
                var max = db.ApplicationRecipts.Max(x => x.ReciptNo);
                var recipt = Convert.ToInt32(max) + 1;
                return recipt;
        ...
        public tbldetail CompanyName()
        {
            ...
            if (HttpContext.Current.User.IsInRole("Consoller"))
            {
                string a = HttpContext.Current.User.Identity.Name;
                var per = db.tblreceptionists.FirstOrDefault(x => x.rid == a).franchid;
                int fra = Convert.ToInt32(per);
                tbldetail rr = db.tbldetails.Where(x => x.franchid ==fra).FirstOrDefault();
                return rr;
            }
            else if (HttpContext.Current.User.IsInRole("Admin"))
            {
                tbldetail rr = db.tbldetails.FirstOrDefault();
                return rr;
            }
            else
            {
